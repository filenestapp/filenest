...
| Complete long-document processing | Translate or summarize every ordered section of an explicitly full-document request, with progress and coverage evidence | attached-file chat | map-reduce document processor | chunks, chat messages | High on macOS and Windows |
...
| Standard Agent Skills | Activate reusable search, grounded-answer, single-file, and feedback-learning workflows from standard `SKILL.md` packages | Smart Search, Find with Chat, Chat with File, feedback processing | `AgentSkillService`, `ChatService` | bundled/shared/managed skill packages | High on macOS and Windows |
| Progressive skill routing | Load only skill metadata during discovery, then activate full instructions by capability, explicit `$skill-name`, semantic match, or conversation reuse | automatic agent routing | `AgentSkillService` | runtime activation state | High on macOS and Windows |
...
| Skill management and diagnostics | Review, enable, disable, refresh, reveal, or remove standard skills and inspect invalid/shadowed packages | AI Skills settings | `AISkillsSettingsView`, `AgentSkillService` | skill packages + disabled-name setting | High on macOS and Windows |
