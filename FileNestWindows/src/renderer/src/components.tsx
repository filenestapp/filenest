...
import { Archive, AudioLines, Code2, Copy, ExternalLink, File, FileImage, FileText, Film, FolderOpen, RefreshCw, Trash2, X } from 'lucide-react'
import type { AppLanguage, FileCategory, FileRecord } from '../../shared/types'
...
export function FileInspector({ file, language, onClose }: { file: FileRecord; language: AppLanguage; onClose(): void }): React.JSX.Element {
  ...
  const generateSummary = async (): Promise<void> => { setBusy(true); try { setSummary(await window.fileNest.summarizeFile(file.id)) } finally { setBusy(false) } }
  return (
    <aside className="inspector" aria-label={t('File Preview')}>
      <header className="inspector-header"><div className="inspector-title"><FileTypeIcon file={file} /><strong title={file.name}>{file.name}</strong></div><IconButton label={t('Cancel')} onClick={onClose}><X size={18} /></IconButton></header>
      <div className="inspector-scroll">
        <section className="inspector-section"><div className="field-label">Relevance</div><span className="status-badge success">● {t('Best Match')}</span></section>
        <section className="inspector-section"><div className="field-label">Location</div><p className="path-text">{file.path}</p><button className="text-action" onClick={() => void window.fileNest.showInExplorer(file.path)}><FolderOpen size={15} />{t('Show in File Explorer')}</button></section>
        <section className="inspector-section detail-grid"><div><div className="field-label">{t('Modified')}</div><p>{formatDate(file.mtime, language)}</p></div><div><div className="field-label">{t('Category')}</div><p>{t(categoryLabels[file.category])} · {formatBytes(file.size)}</p></div></section>
        <section className="inspector-section"><div className="field-label">Quick Actions</div><div className="stacked-actions"><button className="text-action" onClick={() => void window.fileNest.openFile(file.path)}><ExternalLink size={15} />{t('Open')}</button><button className="text-action" onClick={copyPath}><Copy size={15} />{t('Copy File Path')}</button></div></section>
        ...
        <section className="inspector-section danger-zone"><button className="text-action danger" onClick={() => { if (confirm(`${t('Delete')} ${file.name}?`)) void window.fileNest.trashFile(file.id).then(onClose) }}><Trash2 size={15} />Move to Recycle Bin</button></section>
      </div>
    </aside>
  )
}
...
function Preview({ file, url }: { file: FileRecord; url: string | null }): React.JSX.Element {
  if (!url) return <div className="preview-empty"><FileText size={30} /><span>No Preview Available</span></div>
