...
### 未做（明确的后续工作，见第 8 节）
- 当前有 59 项测试，覆盖向量存储/文件级去重/版本化提交、真实 PDF 抽取、文件稳定性、SHA-256、中英文分块、索引状态/内容变化拒绝提交/同文件并发、聊天降级/引用持久化/语义空结果回退、设置持久化与 Provider 切换、规则语义、SQLite CRUD/外键级联、Provider HTTP 边界、文件移动与 DB 路径回写/失败回滚、watcher 重复事件/失败重试/停止竞态/目录重建/启停循环/动态多目录/快速配置变化，以及 `AppState.refresh()`
...
| `OrganizerService.swift` | 173 | 规则决策 + 扩展名回退 + 文件移动（含自定义目标和同名冲突处理）；移动后若 DB 路径更新失败，会把文件移回原位置。`organize(fileId:)` 单个，`runOnce()` 批量 |
...
- **一致性**：如果物理移动成功但 DB 路径更新失败，Organizer 会尝试把文件移回原位置，避免磁盘与数据库路径漂移。
...
2. **继续补核心测试** — 已覆盖索引中文件变化、同文件并发提交和监听配置快速连续变化；下一步覆盖物理回滚本身失败时的可观察错误，以及失败原因/重试次数持久化
