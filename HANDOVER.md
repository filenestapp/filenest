...
### 未做（明确的后续工作，见第 8 节）
- 当前有 57 项测试，覆盖向量存储/文件级去重、真实 PDF 抽取、文件稳定性、SHA-256、中英文分块、索引状态/内容变化拒绝提交、聊天降级/引用持久化/语义空结果回退、设置持久化与 Provider 切换、规则语义、SQLite CRUD/外键级联、Provider HTTP 边界、文件移动与 DB 路径回写/失败回滚、watcher 重复事件/失败重试/停止竞态/目录重建/启停循环/动态多目录/快速配置变化，以及 `AppState.refresh()`
...
| `IndexerService.swift` | 191 | 内容抽取→中英文滑动窗口分块→向量化→入库；提交前复核内容哈希，拒绝写入处理中已变化的旧内容。`indexFile(id:overridePath:)` 是核心方法 |
...
2. **继续补核心测试** — 已覆盖索引中文件变化和监听配置快速连续变化；下一步覆盖同一文件的并发索引提交、物理回滚本身失败时的可观察错误，以及失败原因/重试次数持久化
