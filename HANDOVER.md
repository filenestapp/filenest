...
### 未做（明确的后续工作，见第 8 节）
- 当前有 55 项测试，覆盖向量存储/文件级去重、真实 PDF 抽取、文件稳定性、SHA-256、中英文分块、索引状态、聊天降级/引用持久化/语义空结果回退、设置持久化与 Provider 切换、规则语义、SQLite CRUD/外键级联、Provider HTTP 边界、文件移动与 DB 路径回写/失败回滚、watcher 重复事件/失败重试/停止竞态/目录重建/启停循环/动态多目录，以及 `AppState.refresh()`
...
2. **继续补核心测试** — 已覆盖 `AppState` 状态刷新、watcher 动态多目录边界和 Organizer DB 更新失败回滚；下一步覆盖索引进行中文件再次修改、监听配置快速连续变化，以及物理回滚本身失败时的可观察错误
