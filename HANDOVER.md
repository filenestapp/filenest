...
### 未做（明确的后续工作，见第 8 节）
- 当前有 53 项测试，覆盖向量存储/文件级去重、真实 PDF 抽取、文件稳定性、SHA-256、中英文分块、索引状态、聊天降级/引用持久化/语义空结果回退、设置持久化与 Provider 切换、规则语义、SQLite CRUD/外键级联、Provider HTTP 边界、文件移动与 DB 路径回写、watcher 重复事件/失败重试/停止竞态/目录重建/启停循环，以及 `AppState.refresh()`
...
| 文件 | 行 | 职责 |
|---|---|---|
| `FileWatcherService.swift` | 303 | 监听目录（DispatchSource + 10秒轮询兜底），文件稳定至少2秒后→入DB→触发索引+归类；按路径管理监听源，目录删除时卸载、同路径重建后自动恢复；重复事件去重、失败后允许重试，停止监听后用运行代际阻止旧索引任务继续移动文件 |
...
2. **继续补核心测试** — 已覆盖 `AppState` 状态刷新和 watcher 重复事件/失败重试/停止竞态/目录重建/启停循环；下一步覆盖多个目录并发事件、运行中修改监听目录，以及 Organizer 移动失败后的状态一致性
