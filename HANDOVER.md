...
### 未做（明确的后续工作，见第 8 节）
- 当前有 61 项测试，覆盖向量存储/文件级去重/版本化提交、真实 PDF 抽取、文件稳定性、SHA-256、中英文分块、索引状态/内容变化拒绝提交/同文件并发、聊天降级/引用持久化/语义空结果回退、设置持久化与 Provider 切换、规则语义、SQLite CRUD/外键级联、Provider HTTP 边界、文件移动与 DB 路径回写/失败回滚/回滚失败可观察性、watcher 重复事件/失败重试/停止竞态/目录重建/启停循环/动态多目录/快速配置变化，以及 `AppState.refresh()`
...
| `AccelerateVectorStore.swift` | 256 | 向量检索。向量以 BLOB 存 SQLite，用 vDSP 做 cosine 检索；入库前拒绝非有限、维度不一致或低版本的向量提交，检索时每个文件取最高分块得分且只返回一次。**核心方法：`loadAll` / `replace` / `remove` / `search`** |
...
| 文件 | 行 | 职责 |
|---|---|---|
| `FileWatcherService.swift` | 303 | 监听目录（DispatchSource + 10秒轮询兜底），文件稳定至少2秒后→入DB→触发索引+归类；按路径管理监听源，目录删除时卸载、同路径重建后自动恢复；多目录稳定性独立，重复事件去重、失败后允许重试，停止监听后用运行代际阻止旧索引任务继续移动文件 |
| `OrganizerService.swift` | 212 | 规则决策 + 扩展名回退 + 文件移动（含自定义目标和同名冲突处理）；移动失败、DB 更新失败但已回滚、DB 更新且回滚失败会抛出不同的 `OrganizerError`。`organize(fileId:)` 单个，`runOnce()` 批量 |
| `IndexerService.swift` | 261 | 内容抽取→中英文滑动窗口分块→向量化→入库；提交前复核内容哈希，同一文件用运行代际保证最新调用获胜。`indexFile(id:overridePath:)` 是核心方法 |
...
- **一致性**：如果物理移动成功但 DB 路径更新失败，Organizer 会尝试把文件移回原位置，避免磁盘与数据库路径漂移；调用方会收到“已回滚”错误。若物理回滚也失败，则抛出独立错误，避免磁盘与 DB 已漂移却只留在日志里。
...
- **目录恢复**：监听源按标准化路径管理；删除/重命名事件会释放旧文件描述符，轮询会对账当前配置与磁盘状态，并在目录重新出现时自动挂载新源。
- **多目录隔离**：稳定性观察只清理当前扫描目录下已消失的文件，不能用一个目录的快照删除其他目录的观察记录。
- **注意**：watcher 的 `queue` 必须保持串行，不要重新从其他队列直接调用 `scanDirectory`。

### 7.8 索引提交必须对应同一版文件内容
内容抽取和 embedding 之间存在异步等待，源文件可能在处理中被再次写入。
- **现状**：Indexer 在无文本提交或向量入库前重新计算 SHA-256；与提取前哈希不一致或文件消失时返回失败，保持 `indexedAt = nil` 且不提交旧向量。
- **并发**：每个文件维护当前索引代际；新调用会取代旧任务，DB 写入在代际锁内复核，VectorStore 的串行队列也会拒绝低 revision 的替换或删除。
- **重试**：watcher 会清除该次事件的去重状态，后续稳定扫描重新提取并索引最新内容。

### 7.9 为什么不用 sqlite-vec / vec1
...
2. **继续补核心测试** — 已覆盖索引中文件变化、同文件并发提交、监听配置快速连续变化和 Organizer 回滚失败可观察性；下一步覆盖失败原因/重试次数持久化
